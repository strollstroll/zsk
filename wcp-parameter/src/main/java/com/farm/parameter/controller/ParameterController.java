...
public class ParameterController extends WebUtils {
	...
	public List<Map<String, Object>> queryallForEasyUi(DataQuery query,
		...
		try {
			List<Map<String, Object>> propertys = EasyUiUtils.formatPropertygridData(
							parameterServiceImpl.getAllParameters(), 
							"NAME", "PVALUE", 
	...
	public List<Map<String, Object>> userQueryallForEasyUi(DataQuery query,
		...
		try {
			List<Map<String, Object>> propertys = EasyUiUtils.formatPropertygridData(
							parameterServiceImpl.getUserParameters(getCurrentUser(session).getId()), 
							"NAME", "PVALUE",
							"DOMAIN", "VTYPE", "RULES", "ID");
			return propertys;
		} catch (Exception e) {
	...
	public Map<String, Object> editSubmitByPValue(String ids, HttpSession session) 
	{
		...
		if(ids==null||ids.isEmpty()) {
			return ViewMode.getInstance().setError("参数列表为空！").returnObjMode();
		}
		// end, 2017-12-13
		try {
			...
			for (int i = 0; i < paraArrays.length; i++) {
				...
				parameterServiceImpl.setValue(parameterServiceImpl.getEntity(id).getPkey(), 
						value,
