...
public class FarmDocgroupDao extends HibernateSQLTools<FarmDocgroup>implements FarmDocgroupDaoInter {
	...
	public FarmDocgroup selectFarmDocgroupByGroupName(String groupName) {
/*		Session session = sessionFatory.getCurrentSession();	
		SQLQuery sqlquery = (SQLQuery)session.createSQLQuery("select * from farm_docgroup where GROUPNAME=?").setString(0, groupName);
		FarmDocgroup fdg=new FarmDocgroup();
		fdg=null;
		if(sqlquery.list().size()>0) {
			fdg=(FarmDocgroup) sqlquery.list().get(0);
		}
		return fdg;*/
		
		
		Session session = sessionFatory.getCurrentSession();
		SQLQuery sqlq=(SQLQuery)session.createSQLQuery("select ID from farm_docgroup where GROUPNAME=?").setString(0, groupName);
		FarmDocgroup fd=new FarmDocgroup();
		fd=null;
		if(sqlq.list().size()>0) {
		String id =(String) sqlq.list().get(0);
		fd=(FarmDocgroup) session.get(FarmDocgroup.class, id);
