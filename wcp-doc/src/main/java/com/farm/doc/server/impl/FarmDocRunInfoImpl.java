...
public class FarmDocRunInfoImpl implements FarmDocRunInfoInter {
	...
	//文档个人访问次数
	@Resource
	private FarmDocBrowseDaoInter farmDocBrowseDao;
	...
	@Override
	@Transactional
	public void browseDoc(String docId, LoginUser user) {
		//控制用户访问的计数器工具，用来计算一个KEY在一定时间内是否已经被标记（如控制相同用户不重复计算文章的访问量）
		WebVisitBuff vbuff = WebVisitBuff.getInstance("KNOW", 1000);
		Doc doc=farmDocDao.getEntity(docId);
		String userId=user.getId();
		String userName=user.getName();
		FarmDocBrowse docBrowse=new FarmDocBrowse();
		docBrowse=null;
		docBrowse=farmDocBrowseDao.getEntityByUserIdAndDocId(userId, docId);
		if(vbuff.canVisite(docId+userId)) {
		if(docBrowse!=null) {
			docBrowse.setBrowsenum(docBrowse.getBrowsenum()+1);
			farmDocBrowseDao.editEntity(docBrowse);
		}else{
			FarmDocBrowse docBrowse1=new FarmDocBrowse();
			docBrowse1.setCusername(userName);
			docBrowse1.setDocname(doc.getTitle());
			docBrowse1.setBrowsenum(1);
			docBrowse1.setDocid(docId);
			docBrowse1.setUserid(userId);
			farmDocBrowseDao.insertEntity(docBrowse1);
		}
		}
	}
	//访问次数排名
	public List<FarmDocBrowse> getGoodBrowseUsers(String docId){
		List<FarmDocBrowse> list=farmDocBrowseDao.getEntityByDocId(docId);
		return list;
	}
	...
	public List<DocBrief> getNewKnowList(int pagesize) {
		...
		for (DocBrief node : result) {
			// remyxo,2017-08-22, comment out this code line,
			// this will format to 'yyyy-MM-dd hh:mm:ss', and cause <PF:FormatTime ...> error
