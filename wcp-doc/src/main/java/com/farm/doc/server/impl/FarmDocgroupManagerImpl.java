...
public class FarmDocgroupManagerImpl implements FarmDocgroupManagerInter {
	...
	public FarmDocgroup editFarmDocgroupEntity(FarmDocgroup entity, LoginUser user) {
		...
		// remyxo, 2017-11-29
		String oldimg = entity2.getGroupimg();  // old group img
		String newimg = entity.getGroupimg();   // new group img
		//log.debug("***** FarmDocgroupManagerImpl.editFarmDocgroupEntity: oldimg=" + oldimg + ", newimg=" + newimg);
		if(!newimg.equals(oldimg)) {    // 重置小组头像
			//farmFileServer.cancelFile(oldimg);  // 置为“临时”
			farmFileServer.delFile(oldimg, null);
			log.debug("***** FarmDocgroupManagerImpl.editFarmDocgroupEntity: old imgid(" + oldimg + ") deleted.");
		}
		// end, 2017-11-29
		
		farmFileServer.submitFile(newimg);
		entity2.setGroupimg(newimg);
	...
	public FarmDocgroup editDocGroup(String id, String groupname, String grouptag, String groupimg, boolean joincheck,
		...
		farmFileServer.delFile(entity2.getGroupimg(),null);
		// end, 2017-12-01
		farmFileServer.submitFile(groupimg);
	...
	@Override
	@Transactional
	public FarmDocgroup getFarmDocGroupName(String groupName) {
		return farmDocgroupDao.selectFarmDocgroupByGroupName(groupName);
	}
}
