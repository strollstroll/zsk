...
public class UserServiceImpl implements UserServiceInter {
	...
	public void editCurrentUser(String id, String name, String photoid, String orgid) {
		...
		if (showOrg != null && !showOrg.isEmpty() && showOrg.equals("true") && orgid != null && !orgid.isEmpty()) {
			...
			// remyxo, 2017-12-12
			if(userorg==null) {		// generally, only for sysadmin
				log.debug("***** UserServiceImpl.editCurrentUser: no userorg, create & insert ...");
				userorg = new Userorg("", orgid, user.getId());   // generally, only for sysadmin
				userorgDaoImpl.insertEntity(userorg);             // insert entity
			} // end, 2017-12-12
			else {
