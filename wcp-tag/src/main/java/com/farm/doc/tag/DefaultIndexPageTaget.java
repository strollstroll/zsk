...
public class DefaultIndexPageTaget extends TagSupport {
	...
	public static String getDefaultIndexPage() {
		...
		String defpage;
		try{
			defpage = FarmParameterService.getInstance().getParameter("config.index.defaultpage");
		}catch(Exception e) {
			log.error("***** parameter config.index.defaultpage not configured, use default websearch/PubHome.html");
			defpage = "websearch/PubHome.html";
		}
		return defpage;
		// end, 2017-10-24
	}
