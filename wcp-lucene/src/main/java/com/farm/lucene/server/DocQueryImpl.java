...
public class DocQueryImpl implements DocQueryInter {
	...
	public static DocQueryInter getInstance(File indexFile) {
		...
		docQuery.indexFiles = new ArrayList<File>();
	...
	// remyxo , 2017-12-16, only used by test code, app will use queryByMultiIndex.
	@Override
	public IRResult query(String Iql, int currentPage, int pagesize) throws Exception {
		...
		try {
			...
			log.info("***** kyewords=["+iqlAnalyse.getLimitValue()+"]");
			
			QueryParser mqp = new MultiFieldQueryParser(Version.LUCENE_36,  // remyxo, 2017-12-16, old is LUCENE_35
					iqlAnalyse.getLimitTitle(), new IKAnalyzer());
			mqp.setDefaultOperator(QueryParser.AND_OPERATOR);  // remyxo, 2017-12-16
			Query qp = mqp.parse(iqlAnalyse.getLimitValue().replaceAll(",", " "));  // keywords
			// 在索引器中使用IKSimilarity相似度评估器
			// searcher.setSimilarity( new IKAnalyzer())
			// 使用IKQueryParser查询分析器构造Query对象
			// 开始排序----------------------------------------------------
			Sort sort = iqlAnalyse.getSortTitle();
			TopFieldDocs Docs = searcher.search(qp, Integer.valueOf(iqlAnalyse.getMaxTopNum()), sort);
			// 开始结束----------------------------------------------------
			// // 搜索相似度最高的5条记录
			// TopDocs topDocs = isearcher.search(query, Integer.valueOf(iqlAnalyse.getMaxTopNum()));
			ScoreDoc[] sco = Docs.scoreDocs;
			
			int totleSize = sco.length; // will subDoc() filter some docs ?
			...
			if (ArrayUtils.contains(fields, title)) {
				...
				// 做标题关键字标红
				{
					highlighter.setTextFragmenter(new SimpleFragmenter(
									Integer.valueOf(IndexConf.getString("config.file.lucene_highLight_maxnum"))));
					...
					analyzer.close();    // remyxo, 2017-12-02: Resource leak: analyzer is never closed.
				}
				if (highLight != null) {
					value = highLight;
				}
			...
			throws Exception 
	{
		...
		try {
			...
			log.info("***** search keywords=["+iqlAnalyse.getLimitValue()+"]");

			// 查询构造  
			QueryParser mqp = new MultiFieldQueryParser(Version.LUCENE_36,   // remyxo, old is LUCENE_35
					iqlAnalyse.getLimitTitle(), 
					new IKAnalyzer());
			mqp.setDefaultOperator(QueryParser.AND_OPERATOR);  // remyxo, 2017-12-16
			Query qp = mqp.parse(iqlAnalyse.getLimitValue().replaceAll(",", " "));  // remyxo, add replaceAll
			// 开始排序----------------------------------------------------  
			Sort sort = iqlAnalyse.getSortTitle();
			// 索引文件集合  
			...
			TopFieldDocs docs = searcher.search(qp, Integer.valueOf(iqlAnalyse.getMaxTopNum()).intValue(), sort);
			// 开始结束----------------------------------------------------
			// // 搜索相似度最高的5条记录
			...
			ScoreDoc[] sco = docs.scoreDocs;
			List<ScoreDoc> scolist = new ArrayList<ScoreDoc>();
	...
	{
		return queryByMultiIndex(Iql, currentPage, pagesize, 
