								...
								<div class="form-group">
									...
									<div class="row">
										<div class="col-md-12">
											...
											<input type="hidden" id="imgFileId" name="imgFileId" value="">
											...
											<button type="button" id="delImg" class="btn btn-info btn-sm"
												title="删除图片" onclick="_delImg();" style="visibility: hidden;">
												删除图片</button>
										</div>
									</div>
								</div>
				...
				.uploadify(
						{
							...
							"onUploadSuccess" : function(file, data, response) {//队列中的每个文件上传完成时触发一次
								...
								$('#file_' + obj.id).append('&nbsp;');
								$('#file_' + obj.id).append(
										'<a href="javascript:void(0)" style="color: green;" name="delFile" onclick="removeFile(\''
												+ obj.id + '\');">删除</a>');
								$('#file_' + obj.id).append('&nbsp;&nbsp;');
