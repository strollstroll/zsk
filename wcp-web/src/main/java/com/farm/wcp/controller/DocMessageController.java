...
public class DocMessageController extends WebUtils {
	...
	private static final Logger log = Logger.getLogger(DocMessageController.class);  // remyxo, 2017-12-12
	...
	public ModelAndView docMessage(Integer num, String docid, HttpSession session, HttpServletRequest request) {
		try {
			int hotnum;
			try {
				hotnum = Integer.valueOf(FarmParameterService.getInstance().getParameter("config.sys.hotdocs.show.num"));
			}catch (Exception e) {
				log.warn("DocMessageController.docMessage: parameter \"config.sys.hotdocs.show.num\" not configured, use default 10");
				hotnum = 10;
			}
			// 加载常用文档
