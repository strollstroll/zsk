...
public class DocGroupController extends WebUtils {
	...
	public ModelAndView addCommit(FarmDocgroup group, HttpSession session) {
		//小组名称重名校验
		if(farmDocgroupManagerImpl.getFarmDocGroupName(group.getGroupname().trim())!=null) {
			Map<String,String> links=new HashMap<String,String>();
			links.put("返回","javascript:history.go(-1);");
			return ViewMode.getInstance().putAttr("MESSAGE", "小组名称重复！请更换").putAttr("LINKS", links)
	...
	public ModelAndView show(String typeid, Integer pagenum, String groupid, HttpSession session,
		...
		int docnum = farmDocgroupManagerImpl.getGroupDocNum(groupid)-1;//由于id是从1开始的所有要文章数量要减1
		// 获得小组最新知识
		mode.putAttr("docs",
