...
import com.farm.doc.server.FarmDocRelationManagerInter;  // remyxo,2017-11-01
...
public class DocController extends WebUtils {
	...
	@Resource
	private FarmDocRelationManagerInter farmDocrelationManagerImpl;  // remyxo,2017-11-01
	@Resource
	...
	public ModelAndView showDoc(@PathVariable("docid") String docid, HttpSession session, HttpServletRequest request)
		...
		try {
			...
			//用户访问记录
			farmDocRunInfoImpl.browseDoc(docid,getCurrentUser(session));
			List<FarmDocBrowse> list=farmDocRunInfoImpl.getGoodBrowseUsers(docid);
			page.putAttr("goodBrowseUsers", list);
			...
			if (doc.getDoc().getDomtype().equals("1")) {    //文档知识
				// remyxo, 2017-11-01
				Object relationdocs = this.farmDocrelationManagerImpl.getDocRelationDoc(docid);
				page.putAttr("RELATIONDOCS", relationdocs);
				// end of add
				return page.returnModelAndView(ThemesUtil.getThemePath() + "/know/view");
			}
			if (doc.getDoc().getDomtype().equals("5")) {      //文件知识
	...
	public ModelAndView delCommit(String id,
			HttpServletRequest request,  /* remyxo ,2017-12-13 */
			HttpSession session) {
		try {
			// remyxo, 2017-11-20, this should before deleteDoc()
			String title = farmDocManagerImpl.getDoc(id).getDoc().getTitle();
			String imgid = farmDocManagerImpl.getDoc(id).getDoc().getImgid();
			if(imgid != null) {
				farmDocManagerImpl.delImg(imgid);
				log.debug("***** /webdoc/FLDelKnow, farmDocManagerImpl.delImg(imgid=" + imgid + ") deleted.");
			}
			// end, 2017-11-20
			...
			// remyxo, 2017-12-13
			aloneApplogServiceImpl.wcplog("删除文档", title,
