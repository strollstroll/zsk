...
public class KnowController extends WebUtils {
	...
	public ModelAndView edit(String docid, HttpSession session) {
		DocEntire doce = null;
		List<Doc> reladocs = null;
		try {
			doce = farmDocManagerImpl.getDoc(docid);
			// 解决kindedit中HTML脚本被转义的问题
			doce.getTexts().setText1(doce.getTexts().getText1().replaceAll("&gt;", "&amp;gt;").replaceAll("&lt;", "&amp;lt;"));
			reladocs = this.farmDocrelationManagerImpl.getDocRelationDoc(doce.getDoc().getId());

		} catch (Exception e) {
			e.printStackTrace();
		}
		List<TypeBrief> types = farmDocTypeManagerImpl.getTypesForWriteDoc(getCurrentUser(session));
		return ViewMode.getInstance().putAttr("doce", doce).putAttr("types", types).putAttr("reladocs", (Object)reladocs)
	...
	public ModelAndView submitAdd(String docgroup, String knowtitle, String knowtype, String text, String knowtag, String writetype, String readtype, 
			String relationdocs, String relationfiles, String fileId,
			...
			HttpSession session) 
	{
		...
		try {
			...
			doc = KnowServiceImpl.creatKnow(knowtitle, knowtype, text, knowtag, POP_TYPE.getEnum(writetype),
					POP_TYPE.getEnum(readtype), docgroup, fileId, getCurrentUser(session));  // remyxo, add fileId,2017-11-20
			// remyxo, 2017-11-01
			for (String docid : parseIds(relationdocs))
		    {
				FarmDocrelation relation = new FarmDocrelation();
				relation.setDocid1(doc.getDoc().getId());
				relation.setDocid2(docid);
				relation.setPstate("1");
				this.farmDocrelationManagerImpl.insertFarmDocRelationEntity(relation, getCurrentUser(session));
		    }
			/* no docnum field in wcp 3.2.1 open source!!
			if (doc.getType() != null) {
				farmDocTypeManagerImpl.refreshDocNum(doc.getType().getId());
			}
			*/
			// end, 2017-11-01
		} catch (Exception e) {
		...
		// remyxo, 2017-12-13
		aloneApplogServiceImpl.wcplog("创建文档", knowtitle,
	...
	public ModelAndView editCommit(String docid, String docgroup, String knowtitle, String knowtype, String text,
			String knowtag, String writetype, String readtype, String editNote, 
			String fileId,	String relationdocs, String relationfiles, 
			HttpServletRequest request,  /* remyxo ,2017-12-13 */
			HttpSession session) 
	{
		DocEntire doc = null;
		try {
			...
			// remyxo, 2017-11-01
			this.farmDocrelationManagerImpl.deleteDocRelationsByDocid(docid, getCurrentUser(session));
			for (String id: parseIds(relationdocs))
			{
		        FarmDocrelation relation = new FarmDocrelation();
		        relation.setDocid1(docid);
		        relation.setDocid2(id);
		        relation.setPstate("1");
		        this.farmDocrelationManagerImpl.insertFarmDocRelationEntity(relation, getCurrentUser(session));
			}
			...
			if (farmDocOperateRightImpl.isDel(getCurrentUser(session), farmDocManagerImpl.getDocOnlyBean(docid))) {
				doc = KnowServiceImpl.editKnow(docid, knowtitle, knowtype, text, knowtag, POP_TYPE.getEnum(writetype),
						POP_TYPE.getEnum(readtype), docgroup, fileId, getCurrentUser(session), editNote);
				// remyxo, 2017-12-13
				aloneApplogServiceImpl.wcplog("修改文档", knowtitle,
						getCurrentUser(session).getLoginname(), "INFO", 
						Thread.currentThread().getStackTrace()[1].getMethodName(),
						this.getClass().getName(), 
						getCurrentIp(request));
