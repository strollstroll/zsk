...
import java.util.List;            // remyxo, 2017-12-12
import java.util.Map;

import javax.annotation.Resource;
import javax.servlet.http.HttpSession;

//import org.slf4j.Logger;			// remyxo, 2017-12-13
import org.apache.log4j.Logger;    	// remyxo, 2017-12-13
...
public class UserController extends WebUtils {
	...
	public ModelAndView editUser(HttpSession session) {
		try {
			...
			if ("true".equals(showOrgStr)) {
				...
				orgs = result.getObjectList(Organization.class);
				//for (Organization org1 : orgs) {
				//	log.debug("***** UserController.editUser: ParentID="+org1.getParentid()+", ID="+org1.getId()+", Name="+org1.getName());
				//}
				// end
			}
			
			return ViewMode.getInstance().putAttr("orgs", orgs)  // remyxo, 2017-12-12
					.putAttr("user", user).putAttr("name", name).putAttr("photoid", photoid)
	...
	public ModelAndView regist(HttpSession session) {
		try { // remyxo, 2017-12-12
			...
			if ("true".equals(showOrgStr)) {
				...
				orgs = result.getObjectList(Organization.class);
				//for (Organization org1 : orgs) {
				//	log.debug("***** UserController.regist: ParentID="+org1.getParentid()+", ID="+org1.getId()+", Name="+org1.getName());
				//}
				// end, 2017-12-12
			}
		...
		}catch(Exception e) {  // remyxo, 2017-12-12
			return ViewMode.getInstance().setError(e.toString()).returnModelAndView(ThemesUtil.getThemePath()+"/user/regist");
		}
	}
	...
	public ModelAndView editCurrentUser(String name, String photoid, String orgid, HttpSession session) {
		try {
			// remyxo, 2017-12-01
			User user = userServiceImpl.getUserEntity(getCurrentUser(session).getId());
			String oldimg = user.getImgid();
			// end, 2017-12-01
			
			userServiceImpl.editCurrentUser(getCurrentUser(session).getId(), name, photoid, orgid);
			
			// remyxo, 2017-12-01
			farmFileManagerImpl.submitFile(photoid);
			if(oldimg != null && !oldimg.equals(photoid)) {
				log.debug("***** UserController.editCurrentUser: old photoid(" + oldimg + ") deleted.");
